...
- `writeTitle`: three to six plain words saying what happens in the opening episode, from the premise, the chosen scene, and the prompt as the creator left it. Written when the series is filmed; the series id is minted from it. Nobody types a title.
- Rendering: root on `minimax/h3-max-turbo/text-to-video` at 9:16; children on `minimax/h3-max-turbo/image-to-video` from the parent's last frame; `prompt_expansion_mode: "disabled"`. Reference-to-video is not used for children: a reference frame is a soft attractor and broke both POV and the seam when tested.
- Models on OpenRouter, routed by throughput: `google/gemini-3.8-flash` for roots, the expander, and choices; `google/gemini-3.5-flash-lite` for next episodes and titles. Override with `ROOT_MODEL`, `CHOICE_MODEL`, `EPISODE_MODEL`, `ROOT_TEMPERATURE`.
