...
- `npm run root` writes and renders an opening episode from `PREMISE` (or `PREMISE_FILE`). `npm run hop` extends a baked leaf by one episode. Both print every prompt as it lands.
- `npm run roots`, `npm run expansions`, `npm run choices`, `npm run episodes` are eval runs. Each writes `report.md` and `run.json` under `evals/<kind>/<stamp>-<model>/`, rewritten after every premise. The flags catch rule breaks; they reward blandness, so read the reports.
