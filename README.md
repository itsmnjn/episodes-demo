...
- `bun run root "zoo"` expands the premise into three scenes and writes a prompt for each. `--render` renders all of them in parallel into a stamped folder under `out/root/`, `--direct` films the premise as written.
