...
- `bun run root "zoo"` expands the premise into three scenes and writes a prompt for each. `--render` films one as a new series on the shelf (`--pick 2 --title "..."`), `--direct` films the premise as written.
- `bun run hop the-invitation 0aa` extends an episode by one (`--move "..."` to choose the move).
